...
<article class="board">
	<table class="boards">
		...
		<c:set var="isCheck" value="${dao.selectRecommend(board.idx, user.nick) }"/>
		<c:set var="goodCnt" value="${dao.selectRecCnt(board.idx, 1) }"/>
		<c:set var="badCnt" value="${dao.selectRecCnt(board.idx, 2) }"/>
		<c:set var="args" value="board_idx=${board.idx }&nick=${user.nick }&isCheck=${isCheck }"/>
